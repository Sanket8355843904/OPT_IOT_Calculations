...
if uploaded_file is not None:
    # Step 1: Read CSV with low_memory=False to avoid dtype warning
    ...
    # DBSCAN clustering to identify field vs road
    eps_m = 3.10  # max neighbor distance in meters
    min_samples = 18
    ...
    # Refine labels based on neighborhood majority
    ...
    # Generate union of alpha-shape hulls for each cluster
    union_hull = None
    alpha = 0.08  # concavity control

    # Store hull info for table display
    hulls_info = []
    ...
    for cluster_id in df['cluster'].unique():
        ...
        # Skip clusters with less than 4 points for alphashape
        if len(points_itm) < 4:
    ...
    # Relabel 'field' points outside hull as 'road'
    ...
    # Plot polygons and prepare hull info for table
    hull_count = 0
    ...
    if union_hull is not None:
        ...
        for polygon in polygons:
            hull_count += 1
            # Convert polygon coords back to lat/lon
            hull_latlon = [transformer_inv.transform(x, y)[::-1] for x, y in polygon.exterior.coords]

            # Calculate area in square meters and convert to gunthas
